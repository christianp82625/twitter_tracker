var hashPassword, saltAndHashPassword; // client-side expose

(function() {

// client-side, used for login
hashPassword = function(password) {
	return hex_sha256(password);
}

// client-side, used for registration/editing
saltAndHashPassword = function(password) {
	var salt = generateSalt();

	return {
		salt : salt,
		password : hex_hmac_sha256(salt, hashPassword(password))
	};
}

// Must return a string. e.g. Date.now() as a number -> hex_hmac_sha256 will gave same result for any date.
function generateSalt() {
	return Date.now() + '';
}

// TODO: Check if license allows inclusion like this.
/*
...
module.exports = {
	...
	hashSaltAndPassword: hex_hmac_sha256
}; // server-side expose
